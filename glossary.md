...
**Lane guide** — A per-lane, consumer-authored document of skills, architecture, or guidelines that the agent working in one lane (`product`, `design`, `engineering`, `qa`, `roadmap`) reads before authoring specs in that lane. Declared in `pdeq.json`'s `laneGuides` object (lane id → path relative to `specsRoot`), committed in the consumer's parent repo, and surfaced by framework behavior baked into the canonical `AGENTS.md` templates — not by editing the symlinked submodule. Distinct from a standing spec (project-wide vs lane-scoped); a file may be both. Closes the Codex/Pi append asymmetry: symlink harnesses with no `@import` append slot get the same per-lane customization as Claude. See `product/lane-guides.md`.
