...
- **Removed-harness cleanup vs. consumer content in the claude wrapper:** when `claude` is dropped, `_cleanup_removed_harnesses` removes its wrapper files — but the wrapper may hold consumer edits. Lane `CLAUDE.md` files are genuine one-line `@import`s and are safe to delete; the **root** `CLAUDE.md` ships a `## Project-Specific Instructions` scaffold (Step 4) the consumer is invited to extend. So a claude wrapper is auto-removed only when nothing but the import (and blank lines) remains; if any non-blank content follows the import, it is **preserved with a warning** rather than deleted. This upholds `FR-harness-agnostic-removed-harness-cleaned`'s guarantee that consumer-authored content is never removed, at the cost of leaving an untouched multi-line scaffold behind (a warned, manually-removable file — never silent data loss). Verified by `TC-harness-agnostic-remove-claude-preserves-instructions`.
